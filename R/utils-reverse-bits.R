
reverse_bit_lookup <- as.raw(
  c())


#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#' Reverse the bits in each element of a raw vector
#'
#' This pure R version is about 8x slower and uses about 16x as much
#' memory as the Rcpp version
#'
#' @param raw_vec raw vector
#' @export
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
raw_reverse_bits <- function(raw_vec) {
  stopifnot(is_raw_vec(raw_vec))
  reverse_bit_lookup[as.integer(raw_vec) + 1L]
}


#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# The 32 bit bit reverse is about 20% slower than the 64 bit version.
# Use the 32 bit version for better compatibility.
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Rcpp::cppFunction(
#   code='
#   RawVector bit_reverse(RawVector x) {
#     unsigned int N = x.size();
#     RawVector reversed(N);
#
# //     static const unsigned char BitReverseTable256[256] = {
# // #   define R2(n)     n,     n + 2*64,     n + 1*64,     n + 3*64
# // #   define R4(n) R2(n), R2(n + 2*16), R2(n + 1*16), R2(n + 3*16)
# // #   define R6(n) R4(n), R4(n + 2*4 ), R4(n + 1*4 ), R4(n + 3*4 )
# //     R6(0), R6(2), R6(1), R6(3)
# // };
#
#
#     for (unsigned int i=0; i<N; ++i) {
#       //reversed[i] = BitReverseTable256[x[i]];                                                        // lookup    1.28ms
#       //reversed[i] = (x[i] * 0x0202020202ULL & 0x010884422010ULL) % 1023;                             // 64bit v1  2.13ms
#       //reversed[i] = ((x[i] * 0x80200802ULL) & 0x0884422110ULL) * 0x0101010101ULL >> 32;              // 64bit v2  1.29ms
#       reversed[i] = ((x[i] * 0x0802LU & 0x22110LU) | (x[i] * 0x8020LU & 0x88440LU)) * 0x10101LU >> 16; // 32 bit    1.58ms
#     }
#
#     return reversed;
#     }')
#
# bit_reverse(as.raw(c(1, 2)))
